...
#rm(list=ls())
#setwd("C:/Users/Megan/GitHub/larsen-dissertation/chpt1_NutrStoich")
setwd("C:/Users/meglarse/GitHub/larsen-dissertation/chpt1_NutrStoich")
...
**Overview**: In this experiment, we tested for growth enhancement with the addtion of N or P to our stoichiometrically amended AN media (Lennon *et al.* 2007; see Larsen *et al.* 2016 Table S1). 

## Summary of Major Results
1. Addition of N or P to our N-limited or P-limited media increased *Synechococcus* in comparison to the control treatment (Figure 1, Table 1). 

*Analysis Notes:*
1. Population growth curve data was collected on a Biotek Synergy Mx instrument loaded with softward version 2.01.12.
2. Growth rate calculations were completed using lab generated code for bacterial growth rate analysis for the above instrument and can be found at ` http://github.com/LennonLab/Growth_Curves`.


\newpage

## *Synechococcus* growth rates with response to nutrient addition
```{r gr-calcs, include = FALSE, eval = FALSE}
# Load dependencies
source("~/Github/Growth_Curves/modified_Gomp.R")

# specify output location if needed

# calculate growth curves
growth.modGomp("./data/NutrGR.csv","csv_test",synergy = F, temp = F, smooth = FALSE)
...
round(tukey.fitN$med.add, digits = 3)
...
x <- xtable(tukey.fitN$med.add, digits = 3)
...
xtable(tukey.fitP$med.add, digits = 3)
...
xtable(tukey.fitN$med.add, digits = 3)
...
write.csv(file = "./supporting-files/data/PL-percGRstats.csv",tukey.fitP$med.add)
#kable(tukey.fitP$med.add[,1:4], format = "latex", digits = 3, 
...
# Population Dynamics: Does nutrient stoichiometry affect temporal dynamics?
...
Statistics for these data include repeated measures anova (RMANOVA),  stability (1/CV),  and cross-correlation analyses on whitened data.
...
```{r,results='hide', fig.height=5, fig.width=6, echo=FALSE,fig.cap="N-Limited Chemostats"}
day.start = -125
day.end = 170
...
```{r,results='hide', fig.height=5, fig.width=6,echo=FALSE,fig.cap="P-Limited Chemostats"}
...
```{r, fig.cap= "phage to cyanobacteria ratios"}
...
for(i in unique(dat.sub$cID)){
  plot(x = dat.sub$day[dat.sub$cID == i], y = dat.sub$syn.abd[dat.sub$cID == i], 
       col = "black", lwd = 2, type = "l", 
       log = "y", 
       ylim = c(10^4.5,10^9),
       xlab = "Time (days)", ylab = "Log Abundance", main = i)
  points(dat.sub$day[dat.sub$cID == i], dat.sub$phage.abd[dat.sub$cID == i], 
       col = "grey", lwd = 2, type = "l")
  abline(v = 0, col = "grey")
  par(new = T)
  
  plot(dat.sub$day[dat.sub$cID == i], dat.sub$p.b.ratio[dat.sub$cID == i], 
       col = "red", lwd = 2, type = "l", axes = F, ylab = NA,xlab=NA, las = 1, 
       ylim = c(0,5000))
       #log = "y")
  axis(side=4)
...
vbN <- avg$NmeanP/avg$NmeanSI
vbP <- avg$PmeanP/avg$PmeanSI
```

```{r,results = 'hide', fig.height=3.75, fig.width=6,echo=FALSE,fig.cap="Control Chemostats"}
day.start <- -125
day.end <- 170
...
plot(avg$day,avg$NmeanC,
     ...
     main = "NL", cex.lab = 1.25, font.lab = 1,
...
plot(avg$day,avg$PmeanC,
       xlim = c(day.start,day.end),ylim = c(1*10^4,10^9), log = "y", xaxt = 'n',yaxt = 'n',
       xlab = "Time (d)", ylab = NA, main = "PL", cex.lab = 1.25, 
       lty = 1, lwd = 2, type = "o", col = "black", bg = "white", 
...
### Treatment (i.e. Exposed) population abundances
...
plot(avg$day,avg$NmeanSI,
       xlim = c(day.start,day.end),ylim = c(1*10^4,10^9), log = "y", xaxt = 'n',yaxt = 'n',
       xlab = "Time (d)", ylab = expression(paste("Abundance (mL"^"-1",")")), 
     cex.lab = 1.25, font.lab = 1, main = "NL",
       lty = 1, lwd = 2, type = "o", col = "black", bg = "white", 
       pch = 21, cex = 1)
...
**RMANOVA for control cyanobacteria**
```{r, eval = FALSE}
...
ar1.syn <- lme(log.abd ~ lim * day, random = ~ 1 | cID, 
...
**RMANOVA for control vs exposed cyanobacteria densities**
```{r, eval = FALSE}
...
ar1.syn <- lme(log.abd ~ lim * day * type, random = ~ 1 | cID, 
            correlation=corAR1(form = ~ 1 | cID),
            data=ts.abd)
#summary(ar1.syn)
...
require(multcomp)
summary(glht(ar1.syn), 
             #linfct=mcp(Material = "Tukey")), 
...
### Heteroskedaskicity (i.e skewness) with treatment data only
...
```{r}
ts.syncN <- phase.sync(syncN$Syn,syncN$Phage)
peaks.N <- peaks(syncN$Syn,syncN$Phage, nrands=100)
peaks.N

ts.syncP <- phase.sync(syncP$Syn,syncP$Phage)
peaks.P <- peaks(syncP$Syn,syncP$Phage, nrands=100)
peaks.P
```
...
colnames(avg.data) <- c("day","lim",
                        ...
                        "PseC","PseI","SseC","SseI")
#write.csv(avg.data, file = "./output/cstat-means.csv", row.names = F)
```
...
bor.cstat.Ncontrol <- data.frame(cid.list,phage.time,bac.time,inf.prob)
```
...
bor.cstat.Ncoevo <- data.frame(cid.list,phage.time,bac.time,inf.prob)
...
bor.cstat.Pcontrol <- data.frame(cid.list,phage.time,bac.time,inf.prob)
```
...
bor.cstat.Pcoevo <- data.frame(cid.list,phage.time,bac.time,inf.prob)
...
colnames(bor.cstat) <- c("cID","phage.time","bac.time","inf.prob","trt","lim")
...
```{r, results='hide',fig.width=6, fig.height=10}
